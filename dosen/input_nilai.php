...
<body>
    ...
    <?php
                $query = "SELECT mahasiswa.nama FROM `mahasiswa` INNER JOIN enrolled_code ON enrolled_code.NIM_mahasiswa=mahasiswa.nim WHERE enrolled_code.id='".$_GET['id']."'";
                $data = $conn->query($query);
                $row = $data->fetch_assoc()
    ?>

    <form action="input_nilai_controller.php?code=<?=$_GET['code']?>&pertemuan=<?=$_GET['pertemuan']?>" method="post">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="pertemuan" value="<?=$_GET['pertemuan']?>" readonly>
        </div>
        <div class="form-group">
            <label>Nilai</label>
